...
MultiProcessArgs::MultiProcessArgs(std::string_view rankBindingPath)
    : rankBindingPath_(rankBindingPath), tagOutput_(false), allowRunAsRoot_(false) {}

MultiProcessArgs &MultiProcessArgs::withHosts(const std::vector<std::string> &hosts) {
  __builtin_trap();
}
...
Binary::Binary(std::shared_ptr<void> handle) : Flatbuffer(handle), binaryId(0), tensorCache(nullptr) {}
std::string Binary::getMlirAsJson() const { __builtin_trap(); }
std::uint32_t Binary::getNumPrograms() const { __builtin_trap(); }
std::string Binary::getProgramInputsAsJson(std::uint32_t programIndex) const { __builtin_trap(); }
const std::pair<std::uint32_t, std::uint32_t> Binary::getProgramMeshShape(std::uint32_t programIndex) const { __builtin_trap(); }
std::string Binary::getProgramName(std::uint32_t programIndex) const { __builtin_trap(); }
std::string Binary::getProgramOpsAsJson(std::uint32_t programIndex) const { __builtin_trap(); }
std::string Binary::getProgramOutputsAsJson(std::uint32_t programIndex) const { __builtin_trap(); }
std::string Binary::getSystemDescAsJson() const { __builtin_trap(); }
bool Binary::isProgramPrivate(std::uint32_t programIndex) const { __builtin_trap(); }
std::unordered_map<std::uint32_t, const ::tt::target::GoldenTensor *> Binary::getDebugInfoGolden(std::string &loc) const { __builtin_trap(); }
...
Layout getLayout(Binary executableHandle, std::uint32_t programIndex, std::uint32_t inputIndex) { __builtin_trap(); }
std::unordered_map<MemoryBufferType, MemoryView> getMemoryView(Device device) { __builtin_trap(); }
...
std::vector<TensorRef> getOpInputRefs(OpContext opContextHandle, CallbackContext programContextHandle) { __builtin_trap(); }
std::string getOpLocInfo(OpContext opContextHandle) { __builtin_trap(); }
std::optional<TensorRef> getOpOutputRef(OpContext opContextHandle, CallbackContext programContextHandle) { __builtin_trap(); }
std::unordered_map<std::uint32_t, Tensor> getOpOutputTensor(OpContext opContextHandle, CallbackContext programContextHandle) { __builtin_trap(); }
...
void launchDistributedRuntime(const DistributedOptions &options) { __builtin_trap(); }
...
void releaseTrace(Device meshDevice, std::uint64_t binaryId, size_t mainProgramId) { __builtin_trap(); }
std::optional<Tensor> retrieveTensorFromPool(CallbackContext programContextHandle, TensorRef tensorRef, bool untilize) { __builtin_trap(); }
...
std::vector<Tensor> toHost(Tensor tensor, bool untilize, bool blocking) { __builtin_trap(); }
void updateTensorInPool(CallbackContext programContextHandle, TensorRef tensorRef, Tensor srcTensor) { __builtin_trap(); }
